...
const CodeSystemList: React.FunctionComponent<CodeSystemProps> = ({ hostConfig, conceptId }) => {
  ...
  return (
    <>
      ...
      {codeSystemResultList.map((codeSystem) =>
        codeSystem.items.map(({ internalId, refsetId, additionalFields: { mapAdvice: advice, mapTarget: code } }) => {
          const { title = '' } = hostConfig.codeSystems?.find((set) => set.id === refsetId) ?? {};

          return (
            <dl className={classNames('col-12 col-sm-6 flex-lg-grow-1', styles.definition)} key={internalId}>
              <dt>{title}</dt>
