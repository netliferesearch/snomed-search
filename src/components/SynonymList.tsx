...
const SynonymList: React.FunctionComponent<SynonymProps> = ({ hostConfig, branch, preferredTerm, conceptId }) => {
  const { t } = useTranslation();
  const request = useAsync(fetchSynonyms, [hostConfig, branch, conceptId]);

  const synonymList = request.result || [];

  const excludePreferredTerm = (synonym: Synonym) => synonym.term !== preferredTerm;
