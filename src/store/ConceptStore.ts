...
const fetchConcepts = async (
  ...
  if (hostConfig.languages) {
    ...
    hostConfig.languages.slice(1).forEach((language) => url.searchParams.append('language', language));
  }
...
export const searchConcepts = async (
  ...
  const [conceptResponse, suggestionResponse, refsetConceptResponse] = await Promise.all([
    query ? fetchConcepts(hostConfig, branch, query, refsetId, offset, limit) : ({} as ConceptResponse),
    query && refsetId ? fetchConcepts(hostConfig, branch, query, undefined, offset, limit) : ({} as ConceptResponse),
    !query && refsetId ? fetchRefsetMembers(hostConfig, branch, undefined, refsetId, offset, Limit.High) : ({} as RefsetConceptResponse),
  ]);

  const excludeRefsetMember = (concept: Concept) => !conceptResponse?.items.map((i) => i.concept.conceptId).includes(concept.conceptId);

  const filteredSuggestionResponse: ConceptResponse = {
    totalElements: suggestionResponse.totalElements,
    items: suggestionResponse.items?.filter((item) => excludeRefsetMember(item.concept)),
  };
